	...
	def fetch(self, media_info):
		...
		if cache_key in self.cache:
			...
			if time.time() - cached_result["timestamp"] < self.cache_ttl:
				self.config.logger.debug(f"Using cached cover art URL: {cached_result['url']}")
	...
	def _calculate_release_score(self, recording, release, media):
		...
		if media.get("artist"):
			...
			if any(self._fuzzy_match(media_artist, name) for name in artist_names):
				score += 100
			elif any(media_artist in name or name in media_artist for name in artist_names):
				score += 70
	...
	def _make_request(self, url, params, method="GET"):
		...
		try:
			response = requests.request(
				...
				url,
				params=params,
				headers={"User-Agent": "VLC-Discord-RP/1.0 (https://github.com/valeriko777/vlc-discord-rp)"},
				timeout=3,
			)
			...
			if response.status_code == 200:
				...
				self.config.logger.debug(f"API request failed: {url} - Status {response.status_code}")
				return None
