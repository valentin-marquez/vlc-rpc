...
class StatusReader:
		...
		try:
			...
			if response.status_code != 200:
				if response.status_code == 404:
					self.config.logger.debug("VLC is not running or HTTP interface is misconfigured")
				elif response.status_code == 401:
					self.config.logger.error("Authentication failed. Check your HTTP password.")
				else:
					self.config.logger.error(f"Failed to get VLC status: HTTP {response.status_code}")
				return None
			...
			self.config.logger.debug("VLC is not running or HTTP interface is not accessible")
			return None
	...
	def _convert_vlc_status(self, vlc_status):
		...
		if any(stream.get("Type") == "Video" for stream_name, stream in category.items() if stream_name != "meta"):
		...
		if meta:
			media_info["media"]["title"] = meta.get("title", meta.get("filename", "Unknown"))
	...
	def check_vlc_status(self):
		...
		try:
			...
			if response.status_code == 200:
				...
				return False, "VLC is running but authentication failed (incorrect password)"
			elif response.status_code == 404:
				return False, "VLC is running but the HTTP interface is not properly configured"
			else:
				return False, f"VLC returned unexpected status code: {response.status_code}"
