...
class VideoDetector:
    ...
    def _detect_content_type(self, title):
        ...
        if (
            "[" in clean_title
            and "]" in clean_title
            or re.search(r"\.(sub|dub)\.", clean_title, re.IGNORECASE)
        ):
    ...
    def _clean_title(self, title):
        ...
        title = re.sub(
            r"(-[A-Za-z0-9]+|\d{3,4}p|x264|x265|HEVC|WEB-DL|BluRay|WEBRip)$", "", title
        )
        return title
    ...
    def _fetch_content_image(self, title, content_type, metadata):
        if content_type == "tv_show" and metadata.get("show_name"):
            ...
            if metadata.get("year"):
                search_term = (
                    f"{metadata['movie_name']} {metadata['year']} movie poster"
                )
            else:
            ...
            for script in soup.find_all("script"):
                if script.string and "AF_initDataCallback" in script.string:
                    img_matches = re.findall(
                        r"https?://\S+?\.(?:jpg|jpeg|png)", script.string
                    )
                    for img_url in img_matches:
                        if not any(
                            x in img_url.lower() for x in ["icon", "emoji", "favicon"]
                        ):
                            return img_url
...
if __name__ == "__main__":
    ...
    print(
        f"TV Show: {result.get('content_type')} - {result.get('content_metadata', {}).get('show_name')}"
    )
    ...
    print(
        f"Anime: {result.get('content_type')} - {result.get('content_metadata', {}).get('anime_name')}"
    )
    print(f"Image URL: {result.get('content_image_url')}")
