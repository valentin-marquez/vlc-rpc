...
class Config:
	...
	def parse_vlc_config(self):
		...
		if not os.path.exists(vlcrc_path):
			...
			self.logger.warning("Please run VLC at least once to create the config file")
			return
	...
	def setup_vlc_config(self, port=None, enable_http=True, password=None):
		...
		try:
			...
			self.logger.info(f"Successfully configured VLC with HTTP port {port_to_set}")
...
if __name__ == "__main__":
	...
	if len(sys.argv) > 1 and sys.argv[1] == "setup":
		try:
			...
			print(f"HTTP Password: {config.HTTP_PASSWORD if config.HTTP_PASSWORD else 'Not set'}")
